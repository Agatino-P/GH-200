...
- **`permissions:` — name one, lose the rest.** Naming any permission sets all unspecified ones to `none`; **`metadata` always stays read**. Unnamed perms do NOT keep their defaults.
  - **Scope → action map:** label/comment a PR = `pull-requests: write` (issue labels = `issues: write`) — **NOT** `contents`. `contents: write` = git-repo writes (push commits, tags, releases) — the high-risk scope in a pwn-request context. `packages: write` to publish GHCR; `id-token: write` for OIDC.
...
- **Recursion guard:** `GITHUB_TOKEN`-pushed commits/PRs don't trigger downstream runs. Need them to? Use a **PAT or GitHub App token** — same swap that gives cross-repo reach also lifts the guard.
  - Exceptions: `workflow_dispatch` & `repository_dispatch` **always** create runs even via `GITHUB_TOKEN`; a `GITHUB_TOKEN`-created PR (opened/synchronize/reopened) **does** create runs but in an **approval-required** state. And `workflow_run` fires off another workflow **completing** (a lifecycle event, not a token-emitted one), so it runs normally — the standard way to **chain** off a run the guard would otherwise suppress.
...
**Self-hosted runners:** attach at repository / organization / enterprise. Validate connectivity with the runner app's `config.sh`/`run.sh --check`; connectivity logs in the runner's `_diag` folder. Definition: a machine you deploy & manage to run Actions jobs. **Scope meaning:** repo = single repo; org = many repos in one org; enterprise = many orgs. **Labels:** auto-applied `self-hosted` + OS (`linux`/`windows`/`macOS`) + arch (`x64`/`ARM`/`ARM64`); `runs-on` matches **cumulatively (must have ALL labels)**; set custom labels at registration via `--labels` (the config script can't relabel an existing runner — use UI/REST). **Runner groups:** org/enterprise only; every org has a **default** group (unspecified at registration → default); access policy defaults to **private repos only** (overridable). **Security:** never use self-hosted on **public repos** (a fork PR can run arbitrary code on your box); not ephemeral by default → use **ephemeral/JIT** runners (one job, then auto-removed) to limit state/secret leakage. Routing: no matching idle runner → job stays queued and **fails after 24 h**.

**GHES + GitHub.com actions:** GitHub Connect = AUTO access; `actions-sync` = MANUAL (air-gapped); none by default. GitHub Connect needs **outbound** to github.com from the **instance AND the runners** (no inbound); no outbound → `actions-sync` only. Security: first use of a github.com action **retires that org/repo namespace** locally (blocks a MITM via a look-alike local repo); admin unretires via Site admin.
...
**`pull_request` vs `pull_request_target` (security).** `pull_request` = merge-commit context, **no fork secrets**, blocked by merge conflict → SAFE for running PR code. `pull_request_target` = base-branch context, **full secrets incl. fork PRs**, not blocked by conflict → DANGEROUS for untrusted PR code ("pwn request"). Both default types: opened/synchronize/reopened. Fork `pull_request` token is **read-only** (nothing to steal — that's why running fork code is safe). The vuln is **not the checkout** — it's **executing** the checked-out fork code inside the privileged job that already holds secrets + write token. `pull_request_target` also **auto-runs** (no first-timer approval) and runs the **base-branch** workflow (checks out base by default). **Safe pattern** when you must build untrusted code AND use secrets: unprivileged `pull_request` builds → uploads **artifact** → privileged `workflow_run` consumes it (treat artifact as **untrusted data**). Recency: `actions/checkout` v7 now **blocks** fork head/merge checkout in these privileged events (`allow-unsafe-pr-checkout` opt-out).
...
**Misc.** No `actions/checkout` ⇒ **empty workspace** (repo files absent). `action.yml` required keys = **name + description + runs**. Publishing a package = **`write:packages`** + registry config + publish logic (`registry_package` event only *reacts to* a publish, types `published`/`updated`). **`GITHUB_TOKEN` default = read-only for packages → add `packages: write` to push.** PAT (classic) scopes: `read:packages` / `write:packages` / `delete:packages` (+`repo` for private). **Public GHCR images pull anonymously** (other registries need auth even when public); new packages default **private**. Repo↔package link: **workflow push auto-links**; a CLI push needs `LABEL org.opencontainers.image.source=<repo-URL>` in the Dockerfile. Dispatch endpoints (both POST): `repository_dispatch` → `/repos/O/R/dispatches`; `workflow_dispatch` → `/repos/O/R/actions/workflows/<id>/dispatches`. **GHEC** = hosted SaaS; **GHES** = self-hosted (GitHub Connect auto / `actions-sync` manual; no Marketplace by default). `star` event = repo starred/unstarred (created/deleted). **Matrix-include §8#1 RESOLVED** (Q073/Q124/Q137 all correct).
