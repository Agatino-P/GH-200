...
| 5 | 2026-07-04 | GHCertified session `2026-07-04-a` (`--after Q040`). Drew Q041–Q150 across two sittings (Q072 skipped — answer-exposed during the Q071 investigation). **Two presenter voids: Q071** (showed 6 of 8 options + wrong cardinality) **and Q081** (omitted Type line on a multi-select) — both MY errors, not tool bugs; shuffle + whole-session verified correct (all 138 match bank). Stopped by learner at Q150 | **103/107 = 96% raw** · **103/105 = 98% adjusted** | D1–D5 (matrix incl. include-counting, artifacts vs cache, action types/metadata/lifecycle, environments + protection rules, OIDC, secret scoping/precedence + 48KB + fork non-inheritance, self-hosted runner scopes/diag, REST API verbs, GHES/GitHub Connect/actions-sync, status-check fns, contexts, github.ref vs sha, reusable-vs-composite, outputs blocks) | 4 | (b)×2 (c)×2 |
...
**STATUS:** Testing phase active — **Pass 1 COMPLETE (GHCertified Q001–Q179, Q157 absent = 178 Q, 2026-06-25–06-29). Pass 2 UNDERWAY as of 2026-07-03**, now running on the new **script-enforced pipeline** in `Practice/` (`make_session.py` / `log_answer.py` / `report.py`, recipe = `Practice/QUIZ-PROTOCOL.md`) — this supersedes the old `present.py`/`grade.py`/drill-log-file regime described in §4.10–§4.17 below (kept for historical context; the scripts now enforce shuffle, grading, and logging directly). `Practice/attempts.jsonl` is the durable system of record (append-only, never hand-edited). Session `2026-07-03-a` drew **Q001–Q040** (Q014 voided — presenter answer-leak): **raw 35/39 (90%), adjusted 35/39 (90%)**. Session `2026-07-04-a` drew **Q041–Q100** (`--after Q040`, learner stopped at Q100): **raw 57/57 (100%), adjusted 100%**, five+ Pass-1 gaps recovered; **two PRESENTER voids (Q071, Q081) — NOT tool bugs**, both from truncated reads that dropped options / the Type line. The shuffle pipeline was fully re-validated (all 138 session questions match the bank exactly; obs 103/106 stand). Q072 skipped (answer-exposed). See §6 Session 5. Two (c) misses (Q022, Q027) from S4 are **repeats of Pass-1 gaps** — high-value review targets. §7b gap study (items 1–21) is still **not yet done** — deferred when the learner chose to start Pass 2 first; revisit scheduling. Tutorials Dojo sampler remains RETIRED (2026-06-29).
...
1. **Continue Pass 2**: scripts now live in `Practice/scripts/` (moved 2026-07-04 for tidiness; run from `Practice/` as `python3 scripts/<name>.py` — data still lives in `Practice/`). To generate the next session run `python3 scripts/make_session.py` — `--exclude-recent-correct 2` (default) skips any question with 2 correct in a row. For strict bank-order continuation use `--after Q150` (S5 stopped at Q150; session `2026-07-04-a` is still open through Q179, so Pass 2 can also simply resume presenting from Q151 in that same file). **Q072 is answer-exposed** (its option list was incidentally seen 2026-07-04) — void or skip it when it comes up. The whole-session integrity check is now a permanent tool: `python3 scripts/validate_session.py <id>` (in the QUIZ-PROTOCOL tooling map).
2. **§7b GAP STUDY is still outstanding** — it was queued after Pass 1 completed (2026-06-29) but Pass 2 started (2026-07-03) before it was done. Revisit: either fold it in now or explicitly defer again — flag this to the learner rather than silently deciding.
