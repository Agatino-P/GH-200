...
1. **Continue Pass 2**: run `python3 make_session.py` (from `Practice/`) to generate the next session — `--exclude-recent-correct 2` (default) skips any question with 2 correct in a row. For strict bank-order continuation use `--after Q100` (S5 stopped at Q100). **Q072 is answer-exposed** (its option list was incidentally seen 2026-07-04) — void or skip it when it comes up. The whole-session integrity check lives at `scratchpad/validate_session.py` (compares each session question's options + correct-answer map to the bank) — worth folding into `Practice/` as a permanent post-generation check if desired.
