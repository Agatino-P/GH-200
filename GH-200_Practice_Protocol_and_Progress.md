...
**Last updated: 2026-07-03 20:59 UTC** *(bump this timestamp on every edit — same convention as the study protocol §3.7. Use `date -u`; this value is the real system clock.)*
...
1. **Start Pass 3**: Pass 2 is COMPLETE, so generate a fresh session with `python3 scripts/make_session.py` (from `Practice/`; scripts live in `Practice/scripts/`, data in `Practice/`). The default `--exclude-recent-correct 2` filter will auto-draw exactly the **56 not-yet-done questions** (preview any time with `python3 scripts/pass_progress.py --to 179`). **Q072 is answer-exposed** (option list seen 2026-07-04) — void or skip when it comes up; a fresh shuffle otherwise clears it. Run `python3 scripts/validate_session.py <id>` right after generation to confirm bank integrity.
