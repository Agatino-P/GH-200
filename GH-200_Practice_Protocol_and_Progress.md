...
1. **Continue Pass 2**: run `python3 make_session.py` (from `Practice/`) to generate the next session — `--exclude-recent-correct 2` (default) will skip any question with 2 correct in a row; after session `2026-07-03-a`, 34 of 178 are already excluded on that basis (see `report.py` output). Use `--after Q040` if you want strict bank-order continuation instead of the recency filter, or omit it to let the filter pick.
