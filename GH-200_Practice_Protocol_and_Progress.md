...
1. **BANK FULLY MASTERED (2026-07-05).** Sessions `-a` (56, 91%/93%), `-b` (15, 100%), `-c` (5, 100%) closed out every lagging question. **178/178 now excluded by the 2-correct filter — a plain `make_session.py` returns 0 questions.** No further drilling required for coverage. **▶ FINAL FULL PASS IN PROGRESS: session `2026-07-05-d` — all 178 questions (generated with `--exclude-recent-correct 0`, validated ALL CLEAN). Answered through Q110 as of 2026-07-06 (Q001–Q028, Q029–Q060, Q061–Q102, Q103–Q110 across four sittings). All 7 misses bucket (a) careless (Q008/Q037/Q050/Q063/Q072/Q101/Q110 — mostly mis-keys/skims from boredom re-drilling a mastered bank); Q031 presenter-voided. RESUME AT Q111 — present via `python3 scripts/present.py --session 2026-07-05-d --next Q110`. **▶ GAP BANK COMPLETE (2026-07-07, session `2026-07-06-a`): 40/52 = 77% raw, 40/43 = 93% adjusted — see §6 row 9.** DIG-LIST for targeted review (learner-requested tracking; misses tagged AMBUSH in `attempts.jsonl` notes): **(1) service containers [1.7]** — learner flagged whole topic: container-vs-host networking (service label as hostname vs localhost+mapped port), health checks via `options:` docker flags, port discovery `job.services.<label>.ports[<container-port>]`; **(2) attestations [5.8]** — permission trio (`id-token: write` + `attestations: write` for binaries), availability (public=any plan / private+internal=GHEC only); **(3) runner admin [4.5]** — proxy = `https_proxy`/`http_proxy`/`no_proxy` env or `.env` in runner dir; runner GROUPS = org+enterprise only (runners = 3 levels); connectivity check = `config.sh --check --url --pat` (⚠ NOT `run.sh` — quizmaster's Q066 gloss was wrong and contaminated Q546); **(4) YAML [1.9]** — alias = verbatim/no-extend; `>-` folded scalar was unknown to learner; **(5) one-offs** — runner IP ranges = `GET /meta` `actions` key; JS action failure = `core.setFailed()`. **AUTHORING FIXES PENDING on the gap bank:** Q519 references "the previous file" (Q518) — not self-contained, breaks on filtered redraw; Q547 is redundant with GHCertified Q107/Q139. Fix before the next gap-bank redraw (safe to edit only while no gap-bank session is open).** (new presenter script, see below), grade via `log_answer.py`.** **▶ PRESENTER FIX (2026-07-06):** re-added `Practice/scripts/present.py` — prints the full next-question block INCLUDING the `**Type:**` line straight from `questions.md` (never touches manifest, no answer leak). Built because the presenter twice nearly mis-graded a multi-select as single by truncating the read before the Type line (Q031 was voided for exactly this). ALWAYS present through `present.py` from now on. Other options: near exam date re-verify the 8 currency-flagged items (2026-07-02 audit) against live docs; or declare ready and book. Three+ passes all clear of the 77% line; residual misses were only (a)/(d).
