...
**Last updated: 2026-07-07 21:11 UTC** *(bump this timestamp on every edit — same convention as the study protocol §3.7. Use `date -u`; this value is the real system clock.)*
...
- **Pass** — every usable question answered **exactly once**: **GHCertified 178** (the **Tutorials Dojo sampler was RETIRED 2026-06-29** — low value / false confidence; see §6). Pass = **178 questions**, accumulated across **as many sessions as it takes**. The pass — not the session — is the unit of scoring and readiness.
...
- **Pass 2 / next pass** — a fresh full run of all **178** (GHCertified), started **only after** the previous pass's gap study is done.
...
13. **Draw order within a pass — IN BANK ORDER (added 2026-06-28).** Draw questions **sequentially by source Q-ID** — GHCertified Q001→Q179 (Q157 absent) — never random question selection. *(TD sampler retired 2026-06-29; no longer in rotation.)* **Option order is still shuffled per §4.10**; the in-order rule governs *which question*, not *option position*. Sequencing makes the §6b coverage ledger a simple **high-water mark** ("done through Q-N"), so resume across sessions is unambiguous and no question is drawn twice within a pass.
...
- **Report both per-session and per-pass scores.** Each session's raw/adjusted score is reported so the learner sees how the sitting went — but **readiness is anchored on the full **178-question** (GHCertified) pass** (per-domain breakdown taken across the whole pass), because a single small session's % is noisy (a "50%" on a 4-question sitting is just two misses). Per-session = progress feedback; per-pass = the readiness signal.
...
| 4 | 2026-07-03 | GHCertified session `2026-07-03-a` — **Pass 2 begins**, new script-enforced pipeline (`make_session.py`/`log_answer.py`/`report.py` per `QUIZ-PROTOCOL.md`, live-shuffled at generation time, `--exclude-recent-correct 2`). Drew Q001–Q040 (Q014 voided — presenter leaked answer via commentary before commitment) | **35/39 = 90% raw** · **35/39 = 90% adjusted** | D1 (permissions, triggers, jobs, matrix, reusable workflows, concurrency) | 4 | (a)×2 (c)×2 |
| 5 | 2026-07-04..07-05 | GHCertified session `2026-07-04-a` (`--after Q040`). Drew Q041–Q179 across three sittings — **PASS 2 COMPLETE** (Q072 skipped — answer-exposed during the Q071 investigation). **Two presenter voids: Q071** (showed 6 of 8 options + wrong cardinality) **and Q081** (omitted Type line on a multi-select) — both MY errors, not tool bugs; shuffle + whole-session verified correct (all 138 match bank). | **129/135 = 96% raw** · **129/133 = 97% adjusted** | D1–D5 full breadth (matrix include-counting, artifacts vs cache, action types/metadata/lifecycle, environments + protection rules, OIDC flow, secret scoping/precedence + 48KB + fork non-inheritance, self-hosted runner scopes/diag/groups/labels/IP-allow, REST verbs, GHES/Connect/actions-sync, contexts, github.ref vs sha, reusable-vs-composite, outputs blocks, pull_request vs pull_request_target, packages publishing) | 6 | (a)×1 (b)×2 (c)×3 |
| 6 | 2026-07-05 | GHCertified session `2026-07-05-a` — **PASS 3 COMPLETE** (single sitting). Pre-generated 56-question not-yet-mastered subset (122 recently-correct auto-excluded via `--exclude-recent-correct 2`); all 56 drawn + validated ALL CLEAN vs bank. Q014 & Q072 (prior void/skip) re-presented on fresh shuffle — both handled clean. Learner requested a mid-session bank re-check on Q014 (loose-wording matrix item); `validate_session.py` re-confirmed ALL CLEAN and the bank rationale (matrix-over-reusable-workflow via `strategy.matrix` on a `uses:` job) — kept excluded as (d). | **51/56 = 91% raw** · **51/55 = 93% adjusted** | The 56 not-yet-mastered across D1–D5 (defaults, matrix throttling, env/if scopes, status-fn gate, secret precedence, reusable-vs-composite, outputs blocks, GHES/Connect, github.ref by event, OIDC, packages publish scope, dispatch endpoints, runner labels/IP-allow, action.yml keys, checkout-required, environment reviewers) | 5 | (a)×3 (c)×1 (d)×1 |
| 7 | 2026-07-05 | GHCertified session `2026-07-05-b` — mop-up of the 15 still-lagging questions (163 auto-excluded); validated ALL CLEAN. Every question recovered — including all 5 Pass-3 misses (Q014/Q081/Q098/Q125/Q158, now at 1 consecutive correct) and the 10 wrong-then-right stragglers (now 2-in-a-row → mastered). | **15/15 = 100% raw** · **15/15 = 100% adjusted** | The 15 not-yet-mastered: matrix/defaults, workflow-reuse vs copy, cron weekdays, re-run perms, default-var context, env levels, GHES/Connect, github.ref on push, on-merge idiom, Docker action files | 0 | — |
| 8 | 2026-07-05 | GHCertified session `2026-07-05-c` — final 5 (Q014/Q081/Q098/Q125/Q158), validated ALL CLEAN. All landed → each now 2-consecutive-correct. **BANK FULLY MASTERED: 178/178 excluded by `--exclude-recent-correct 2`; `make_session.py` draws 0.** | **5/5 = 100% raw** · **5/5 = 100% adjusted** | matrix-over-reusable-wf, workflow reuse, cron weekdays, env levels, on-merge idiom | 0 | — |
| 9 | 2026-07-05..06 | GHCertified session `2026-07-05-d` — **FINAL FULL PASS (PASS 4/freshen) COMPLETE**: all 178 questions re-drilled from scratch (`--exclude-recent-correct 0`), across five sittings on two machines (Q001–Q110 + Q111–Q179). Q031 presenter-voided (multi-select shown as single → `present.py` built as the structural fix). **All 9 misses bucket (a) careless** (Q008/Q037/Q050/Q063/Q072/Q101/Q110/Q144/Q158 — mis-keys and boredom-skims re-drilling mastered material); zero (b)/(c)/(d) — no knowledge gaps in the whole bank. | **168/177 = 95% raw** · **168/177 = 95% adjusted** | full bank D1–D5 | 9 | (a)×9 |
| 10 | 2026-07-06..07 | **GAP BANK session `2026-07-06-a` — COMPLETE, all 52 authored questions (Q501–Q552), validated ALL CLEAN.** First drill of the untrained/thin objectives. Blocks: attestations 7/9, script injection 8/8, YAML anchors 6/7, job summaries 5/5, service containers 4/6, IP allow lists 3/4, runner admin 5/7, action troubleshooting 4/5. Misses tagged AMBUSH in notes where material was untrained. Quizmaster contamination on Q546 (my Q066 gloss wrongly said `run.sh --check`; correct is `config.sh --check`). | **40/52 = 77% raw** · **40/43 = 93% adjusted** | 5.8 attestations, 5.3 injection, 1.9/2.3 YAML, 1.14 summaries, 1.7 services, 4.4 IP lists, 4.5 runner admin, 3.3 troubleshooting | 12 | (a)×1 (b)×9 (c)×2 |
| 11 | 2026-07-07 | **GAP BANK redo session `2026-07-07-a`** — the 12 row-10 misses (`--qids`), validated ALL CLEAN; preceded by a DIG-LIST topic review in chat. 11 recovered on first re-ask (incl. all 9 AMBUSH items and the Q546 contamination fix). Only miss: Q542 runner-group levels — stem misread (read "runner", missed "groups"), rule itself known; stays failed-last. | **11/12 = 92% raw** · **11/12 = 92% adjusted** | 5.8 attestations, 1.9 YAML alias, 1.7 services, 4.4/4.5 runner admin, 3.3 `core.setFailed` | 1 | (a)×1 |
| 12 | 2026-07-07 | **GAP BANK PASS 2 COMPLETE, session `2026-07-07-b`** — all 52 questions (Q501–Q552), fresh shuffle, validated ALL CLEAN, single sitting. **PERFECT RUN — every block clean**, incl. all five DIG-LIST topics and Q542 (presenter bolded "groups" in the stem as a misread-nudge; learner chose to keep the grade — knowledge shown twice before). **GAP BANK FULLY MASTERED: 52/52 at 2-consecutive-correct; only the 9 row-9 careless (a) GHCertified misses remain failed-last (220/230 excluded).** | **52/52 = 100% raw** · **52/52 = 100% adjusted** | full gap bank: 5.8 attestations, 5.3 injection, 1.9/2.3 YAML, 1.14 summaries, 1.7 services, 4.4 IP lists, 4.5 runner admin, 3.3 troubleshooting | 0 | — |
| 13 | 2026-07-07 | **GHCertified mop-up session `2026-07-07-c`** — the 9 row-9 careless (a) items (default filter drew exactly those), validated ALL CLEAN. 8 recovered. Only miss: **Q050 secret/variable scoping — now ✗✗, re-bucketed (c)**: picked "specific workflow" over "organization"; keeper: secrets/vars scope to **org / repo / environment — never per-workflow or per-job**. Sole remaining failed-last question. | **8/9 = 89% raw** · **8/9 = 89% adjusted** | dispatch events, GITHUB_OUTPUT, secret scoping, ::debug::, org-reusable components, failure() gating, workflow-templates repo, retention-days, on-merge idiom | 1 | (c)×1 |
| 14 | 2026-07-07 | **Single re-ask session `2026-07-07-d`** — Q050 on a fresh shuffle after the (c) triage. Landed: org/repo/environment picked clean over the workflow/job distractors. **FAILED-LAST LIST EMPTY — all 230 questions in both banks correct on last attempt.** | **1/1 = 100%** | 5.2 secret/variable scoping | 0 | — |
...
**Session 4 (2026-07-03, session `2026-07-03-a` — Pass 2 start, 40 drawn Q001–Q040).** First session under the new script-enforced pipeline (`make_session.py` generates + freshly shuffles a session at run time from the raw bank; `log_answer.py` grades letter-vs-letter against the session manifest; `report.py` scores). `--exclude-recent-correct 2` excluded 0 questions going in (nothing yet had 2 correct in a row) — all 178 were eligible; this session drew the first 40 in bank order. **Raw 35/39 = 90%, adjusted 35/39 = 90%** (1 voided, 4 misses: (a)×2, (c)×2, no (b)/(d)). **Q014 voided** — presenter (Claude) editorialized on the correct answer via a parenthetical before the learner committed, a presentation leak under §5; not scored. **Two (c) retention holes, both repeats of prior-pass misses:** Q022 (`workflow_dispatch` triggerable via REST API/CLI, not UI-only — repeat of S2·Q022) and Q027 (no `defaults.env` exists — `defaults` supports only `run`; repeat of S2·Q027, originally bucket (b), now re-classed (c) since the learner confirmed it was studied material forgotten under recall pressure). **Two (a) careless slips** late in the session (Q036 matrix `max-parallel` vs `concurrency`; Q038 `$GITHUB_ENV` surfaces as a plain shell var, not `steps.outputs`) — learner flagged fatigue as the cause for both, session continued anyway by learner's choice. Session generated as `2026-07-03-a` (a fresh session id, not `pass2-*`); next session's `--after Q040` or a plain re-run (now excluding Q001–Q013/Q015–Q021/Q023–Q035/Q037/Q039 per the 2-correct filter) continues Pass 2 in bank order.

**Session 5 (2026-07-04..07-05, session `2026-07-04-a` — `--after Q040`, drew Q041–Q179 over three sittings — PASS 2 COMPLETE).** Continued Pass 2 in bank order (34 excluded by the 2-correct filter). **Raw 129/135 = 96%, adjusted 129/133 = 97%** (137 attempted, 2 presenter-voided, Q072 skipped as exposed). **Six misses: (a)×1 (b)×2 (c)×3** — Q115 (c, re-run needs write, not read), Q121 (b, wrong mental map: default env vars via `github` context not `env`; also missed CI-overwrite caveat), Q132 (b, GHES: `actions-sync` is a CLI not a workflow action, and GHES uses self-hosted not GitHub-hosted runners), Q148 (c, `github.ref` = fully-formed ref, not the SHA), Q158 (c, no `merged` PR activity type — use `types:[closed]` + `if merged==true`), Q162 (a careless, Dockerfile needed when image is BUILT at runtime not when referenced from a registry). Big retention win: ~30+ Pass-1 gaps recovered across the three sittings. Recurring PRESENTER issue: repeatedly truncated the `Type:` line / dropped trailing options (Q071, Q081, Q139, Q147, Q156, Q160, Q171, Q173) — caught most pre-grade; process fix reiterated: read the whole block from the `## Q` header through the `Type:` line + closing `---` in one pass before presenting. **Q071 VOIDED — PRESENTER truncation, NOT a shuffle bug.** What happened: the quizmaster (Claude) presented Q071 having read only 6 of its 8 options and, missing the Type line, wrongly labeled it single-select; the two correct options ("repository secret with same name `API_KEY`" and "environment secret with same name `API_KEY`") were absent *from the presentation only*. The learner correctly sensed the answer was missing ("the name must be the same; enterprise doesn't override org"). Claude then **misdiagnosed this as shuffle-tool corruption and voided it with a false reason — that diagnosis was wrong.** On investigation the session file is correct (Q071 has all 8 options, `multi-select (2 correct)`, manifest `correct = G,H` = the two same-name options), and a **full bank-vs-session integrity check of all 138 questions came back ALL CLEAN** — every option set and every correct-answer mapping matches the master bank exactly (script at `scratchpad/validate_session.py`; run: `parse bank + apply manifest map, strip `>` answer-hint annotations to mirror `strip_key`, compare to questions.md`). **The shuffle pipeline (`shuffle_bank.py` / `make_session.py`) is sound; the 2026-07-02 obs 103/106 verification stands.** The void was re-logged (via `--force`) with the corrected reason and kept — a garbled *presentation* is a legit §5 void, and Q071's answer got discussed here so it should not be re-answered this session; it stays eligible and will resurface with a fresh shuffle later. **Process fix (on the presenter, not the code): read the ENTIRE question block — every option AND the Type line (single vs multi-select N correct) — before presenting.** This failure recurred in the SAME session: Q081 was also presenter-voided because the quizmaster stopped one line short of the Type line, presented a `multi-select (2 correct)` as if single, and the learner answered one letter (D — itself one of the two correct: B,D). Both voids trace to truncated reads. Q072's option list was incidentally seen during the Q071 investigation → Q072 skipped this session and flagged to void/skip when next drawn.
...
**▶ DECISION 2026-06-29 — Tutorials Dojo sampler RETIRED.** After drilling TD Q1–Q29 (Q26 off-syllabus, Q30 dropped): **~24/28 correct**, but the learner judged the source **low-value** — re-treads of GHCertified concepts with looser wording, two actively ambiguous items (Q3 internal-vs-org-repo; Q13 publishing-vs-referencing, both re-bucketed (d)/excluded), and a **false-confidence risk**. **Dropped from Pass 1 scope.** Pass 1 = **GHCertified only (178 Q) = COMPLETE.** TD drill entries **purged from the drill log** (learner decision — keep it GHCertified-only so later analysis needs no manual exclusion); the two useful misses are preserved here in §7. Two genuinely useful TD misses kept → §7: **TD·Q21** (`GITHUB_REF` = triggering ref, not `GITHUB_WORKFLOW` = name; ties §7b#8) and **TD·Q25** (reusable-wf `uses:` needs `{owner}/{repo}/.github/workflows/{file}@{ref}` — owner required).
...
| S4·Q022 | REPEAT of S2·Q022: `workflow_dispatch` IS triggerable via REST API (`POST .../dispatches`) / CLI / UI — not UI-only. Second miss on same concept, now Pass 2 | (c) | 1.1/3.x | Cheat (1A) — high-value repeat, prioritize in gap study |
| S4·Q027 | REPEAT of S2·Q027 (was bucket b): no `defaults.env` exists; `defaults` supports ONLY `run` (shell + working-directory), workflow/job level only. Second miss, learner confirms studied-then-forgotten → re-bucketed (c) | (c) | 1.6/2.x | Cheat keeper — high-value repeat |
...
| S4·(a) | Awareness-only (no review): Q036 mistyped `max-parallel` vs `concurrency` (knew it), Q038 mistyped `$GITHUB_ENV`-as-plain-var vs `steps.outputs` (knew it) — both learner-attributed to end-of-session fatigue | (a) | — | pacing, not gaps |
...
| **Tutorials Dojo** | Free sampler | 30 sampler | ⛔ **RETIRED 2026-06-29** (trialed Q1–Q29; dropped — low value vs GHCertified, ambiguous Q3/Q13, false confidence). Files kept in repo for record only, not in rotation. ~~Sampler verified 2026-06-24~~ (30 Q, **0 wrong keys**). Raw = `gh-200-tutorialsdojo-sampler-bank-2026-06-24.md`; verified = `gh-200-tutorialsdojo-sampler-verified-2026-06-24.md`. ⚠ **Q26 is off-syllabus** (a Microsoft Sentinel/KQL item, bucket d — skip it); soft flags on Q3/Q27/Q29 (Q29 = dated, OIDC not offered). Good secondary cross-check; no position bias but shuffle anyway (§4.10). **Links:** free sampler `https://portal.tutorialsdojo.com/product/free-gh-200-github-actions-practice-exams-sampler/` · full course `https://portal.tutorialsdojo.com/product/gh-200-github-actions-practice-exams/`. (TD markets it as "not exam dump.") Full 140+ bank not yet collected. |
...
**STATUS:** Testing phase active — **Pass 1 COMPLETE** (GHCertified Q001–Q179, Q157 absent = 178 Q, 2026-06-25–06-29). **Pass 2 COMPLETE** (Q001–Q179 across sessions `2026-07-03-a` + `2026-07-04-a`). **Pass 3 COMPLETE as of 2026-07-05** — session `2026-07-05-a` drew the 56 not-yet-mastered questions (122 already-mastered auto-excluded), **raw 51/56 = 91%, adjusted 51/55 = 93%**; 5 misses (a)×3 (Q098/Q125/Q158) (c)×1 (Q081) (d)×1 (Q014 excluded). 4 of 5 misses were careless/bad-question, not conceptual — 50 of the 56 lagging questions recovered to a clean pass. **Mop-up sessions `2026-07-05-b` (15/15) + `2026-07-05-c` (5/5), both 2026-07-05, both 100%.** **BANK NOW FULLY MASTERED: 178/178 questions have 2 consecutive corrects → `make_session.py` draws 0.** Every question has been answered correctly on its two most recent attempts. Readiness signal is unambiguous; remaining prep is optional freshening (regenerate with `--exclude-recent-correct 0` to re-drill the whole bank, or re-verify the 8 currency-flagged items near the exam date). Running on the **script-enforced pipeline** in `Practice/scripts/` (`make_session.py` / `log_answer.py` / `report.py` / `validate_session.py` / `question_status.py` / `pass_progress.py`, recipe = `Practice/QUIZ-PROTOCOL.md`) — supersedes the old `present.py`/`grade.py`/drill-log regime (§4.10–§4.17, historical). `Practice/attempts.jsonl` is the durable system of record (append-only, never hand-edited). **Pass 2 scores:** S4 `2026-07-03-a` Q001–Q040 raw 35/39 (90%); S5 `2026-07-04-a` Q041–Q179 raw 129/135 (96%), adjusted 97%. Two PRESENTER voids (Q071, Q081) — NOT tool bugs (shuffle re-validated, all 138 match bank; obs 103/106 stand); Q072 skipped (answer-exposed) — both resurface next pass. **Pass 2 overall (Q001–Q179): raw 164/174 = 94.3%, adjusted 164/172 = 95.3%** — a ~+22pt jump over Pass 1 (~72%). 10 misses: (a)×3 (Q036/Q038/Q162), (b)×2 (Q121/Q132), (c)×5 (Q022/Q027/Q115/Q148/Q158) — residue is mostly retention-consistency, not conceptual gaps. **Pass 3 preview (per `pass_progress.py --to 179`): of 178 questions, 122 are "done" (2 consecutive correct → auto-excluded) and 56 will be re-presented** (5 never-right, 5 right-then-wrong, 36 wrong-then-right, 7 single-clean-correct, 3 single-wrong). **§7b gap study — considered ADDRESSED through Pass 2** (the weak topics were drilled directly and the score moved; learner's call 2026-07-05). Tutorials Dojo sampler RETIRED (2026-06-29).
...
2. Read `Practice/QUIZ-PROTOCOL.md` for the current session recipe, then this file for narrative state — especially **§0 (definitions + no-data-loss rule)**. The **live high-water mark now lives in `Practice/attempts.jsonl`**, read via `python3 Practice/report.py` (not §6b, which stopped being updated once the script pipeline took over — see note below). Recaps/cheats are in `Study material/` (repo + HD) — ask the learner to upload any needed for a targeted review.

**Next decisions / actions:**
1. **BANK FULLY MASTERED (2026-07-05).** Sessions `-a` (56, 91%/93%), `-b` (15, 100%), `-c` (5, 100%) closed out every lagging question. **178/178 now excluded by the 2-correct filter — a plain `make_session.py` returns 0 questions.** No further drilling required for coverage. **▶ FINAL FULL PASS COMPLETE (session `2026-07-05-d`, 2026-07-05..06, finished on the other machine through Q179): 168/177 = 95% raw & adjusted, all 9 misses bucket (a) careless, zero (b)/(c)/(d) — see §6 row 9. Nothing left to resume in the GHCertified bank.** **▶ GAP BANK COMPLETE (2026-07-07, session `2026-07-06-a`): 40/52 = 77% raw, 40/43 = 93% adjusted — see §6 row 10.** **▶ REDO SESSION `2026-07-07-a` (2026-07-07): all 12 row-10 misses re-drilled after a DIG-LIST topic review — 11/12 = 92%, only Q542 (runner-group levels) missed on a stem misread (a) — see §6 row 11.** **▶ GAP BANK PASS 2 — PERFECT (2026-07-07, session `2026-07-07-b`): 52/52 = 100%, single sitting; GAP BANK FULLY MASTERED at 2-consecutive-correct. DIG-LIST topics all confirmed closed. Remaining failed-last: only the 9 careless (a) GHCertified items from row 9 (Q008/Q037/Q050/Q063/Q072/Q101/Q110/Q144/Q158) — optional mop-up; zero knowledge gaps anywhere. See §6 row 12.** **▶ MOP-UP `2026-07-07-c` (2026-07-07): 8/9 recovered — the ONLY open item in both banks is now Q050 (secret/variable scoping, ✗✗, bucket c): keeper = secrets/vars scope to org / repo / environment, NEVER per-workflow or per-job. Re-ask Q050 next session; otherwise ready to book. See §6 row 13.** **▶ Q050 RE-ASK DONE (2026-07-07, session `2026-07-07-d`): correct on fresh shuffle — FAILED-LAST LIST EMPTY, all 230 questions in both banks correct on last attempt. NOTHING LEFT TO DRILL — declare ready and book. See §6 row 14.** DIG-LIST for targeted review (learner-requested tracking; misses tagged AMBUSH in `attempts.jsonl` notes): **(1) service containers [1.7]** — learner flagged whole topic: container-vs-host networking (service label as hostname vs localhost+mapped port), health checks via `options:` docker flags, port discovery `job.services.<label>.ports[<container-port>]`; **(2) attestations [5.8]** — permission trio (`id-token: write` + `attestations: write` for binaries), availability (public=any plan / private+internal=GHEC only); **(3) runner admin [4.5]** — proxy = `https_proxy`/`http_proxy`/`no_proxy` env or `.env` in runner dir; runner GROUPS = org+enterprise only (runners = 3 levels); connectivity check = `config.sh --check --url --pat` (⚠ NOT `run.sh` — quizmaster's Q066 gloss was wrong and contaminated Q546); **(4) YAML [1.9]** — alias = verbatim/no-extend; `>-` folded scalar was unknown to learner; **(5) one-offs** — runner IP ranges = `GET /meta` `actions` key; JS action failure = `core.setFailed()`. **AUTHORING FIXES — WONTFIX (learner decision 2026-07-07):** Q519 not self-contained (needs Q518's snippet on a filtered redraw) and Q547 redundant with GHCertified Q107/Q139 — both accepted as-is; don't re-raise. Likewise the near-exam re-verification of the 8 currency-flagged items (2026-07-02 audit, notes in the bank file) — **SKIPPED by learner decision** (2-week-old verification is fresh enough); don't re-raise unless the exam date slips months.** **▶ PRESENTER FIX (2026-07-06):** re-added `Practice/scripts/present.py` — prints the full next-question block INCLUDING the `**Type:**` line straight from `questions.md` (never touches manifest, no answer leak). Built because the presenter twice nearly mis-graded a multi-select as single by truncating the read before the Type line (Q031 was voided for exactly this). ALWAYS present through `present.py` from now on. Other options: near exam date re-verify the 8 currency-flagged items (2026-07-02 audit) against live docs; or declare ready and book. Three+ passes all clear of the 77% line; residual misses were only (a)/(d).
2. **§7b GAP STUDY — CLOSED (2026-07-05).** No longer a standalone task: the weak topics were drilled directly through Pass 2 and the score moved from ~72% to ~94% (learner's call). Don't re-flag it; if a specific topic still wobbles in Pass 3, handle it inline via the miss ledger.
3. **Per question:** `log_answer.py` writes each attempt straight to `attempts.jsonl` — no separate manual ledger step needed. Triage every miss into a bucket via `--set-bucket` before moving on (§4 of `QUIZ-PROTOCOL.md`), and roll high-value (b)/(c) repeats into §7 above as they occur.
4. **At session end (NO-DATA-LOSS — §0):** run `report.py` for the raw/adjusted score, write the §6 session row + §7 miss-ledger entries here, bump the timestamp, and re-sync project + repo + HD (commit `attempts.jsonl` + `sessions/<id>/`). Never leave progress only in chat.
5. **Settle parked flags (§8)** opportunistically as questions surface them.

**Note on §6b (coverage ledger):** that table stopped being maintained once the script pipeline (`make_session.py`/`log_answer.py`) took over durable logging (2026-07-03) — it now lags behind `attempts.jsonl`, which is the authoritative source. Treat §6b as a historical snapshot of Pass 1 only; don't append to it going forward. If a future session wants a similarly detailed per-question table for Pass 2, generate it from `attempts.jsonl` rather than hand-maintaining a duplicate.
...
---
*Hand-off #3 — 2026-06-29: Tutorials Dojo sampler RETIRED (learner decision); Pass 1 scope reduced to GHCertified-only (178 Q) = COMPLETE; high-water/RESUME/§4.13/counts updated; RESUME now points to §7b gap study → Pass 2. TD drill entries purged from the drill log (GHCertified-only). Synced: project + repo + HD.*
