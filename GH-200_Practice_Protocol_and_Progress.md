...
| 5 | 2026-07-04 | GHCertified session `2026-07-04-a` (`--after Q040`). Drew Q041–Q071. **Q071 voided — PRESENTER truncation, NOT a tool bug** (quizmaster showed 6 of 8 options + wrong cardinality; shuffle + whole-session verified correct — see analysis). Session **stopped early by learner** | **30/30 = 100% raw** · **30/30 = 100% adjusted** | D2/D4/D5 (artifacts vs cache, action types/metadata, environments, OIDC, secret scoping/precedence, self-hosted runner diag, REST API verbs, GHES/GitHub Connect) | 0 | — |
...
1. **Continue Pass 2**: run `python3 make_session.py` (from `Practice/`) to generate the next session — `--exclude-recent-correct 2` (default) skips any question with 2 correct in a row. For strict bank-order continuation use `--after Q071` (S5 stopped at Q071). **Q072 is answer-exposed** (its option list was incidentally seen 2026-07-04) — void or skip it when it comes up. The whole-session integrity check lives at `scratchpad/validate_session.py` (compares each session question's options + correct-answer map to the bank) — worth folding into `Practice/` as a permanent post-generation check if desired.
