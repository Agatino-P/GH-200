...
| 5 | 2026-07-04 | GHCertified session `2026-07-04-a` (`--after Q040`). Drew Q041–Q100 (Q072 skipped — answer-exposed during the Q071 investigation). **Two presenter voids: Q071** (showed 6 of 8 options + wrong cardinality) **and Q081** (omitted Type line on a multi-select) — both MY errors, not tool bugs; shuffle + whole-session verified correct (all 138 match bank). Stopped by learner at Q100 | **57/57 = 100% raw** · **57/57 = 100% adjusted** | D1/D2/D4/D5 (matrix job count, artifacts vs cache, action types/metadata, environments, OIDC, secret scoping/precedence + 48KB, self-hosted runner scopes/diag, REST API verbs, GHES/GitHub Connect, status-check fns, workflow syntax) | 0 | — |
...
1. **Continue Pass 2**: scripts now live in `Practice/scripts/` (moved 2026-07-04 for tidiness; run from `Practice/` as `python3 scripts/<name>.py` — data still lives in `Practice/`). To generate the next session run `python3 scripts/make_session.py` — `--exclude-recent-correct 2` (default) skips any question with 2 correct in a row. For strict bank-order continuation use `--after Q100` (S5 stopped at Q100; session `2026-07-04-a` is still open through Q179, so Pass 2 can also simply resume presenting from Q102 in that same file). **Q072 is answer-exposed** (its option list was incidentally seen 2026-07-04) — void or skip it when it comes up. The whole-session integrity check is now a permanent tool: `python3 scripts/validate_session.py <id>` (in the QUIZ-PROTOCOL tooling map).
