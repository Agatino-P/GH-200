# GH-200 practice session 2026-07-05-d

Generated 2026-07-05 from `gh-200-ghcertified-bank-full-2026-06-24.md` — 178 questions, options shuffled and re-lettered.
...
- B. The `GITHUB_TOKEN` permissions passed from the caller workflow can be both downgraded and elevated by the called workflow.
- C. The `GITHUB_TOKEN` permissions passed from the caller workflow can be only downgraded by the called workflow.
...
- A. Adding a label to a pull request
- B. Cloning the repository
- C. Committing a file to master branch
...
- C. Workflows are written in any of `.yaml`, `.json` or `.toml` formats
- D. Workflows have to be defined in the `.github/workflows` directory
- E. Workflows can run one or multiple jobs at a time
- F. Workflow can run only one job at a time
- G. Workflows can only be run on a schedule
...
- B. Jobs run based on the order they are defined in the workflow file
- C. Jobs run in sequence
...
- B. the jobs that are dependent on job A are skipped
...
- A. the source branch name starts with `release` but does not end with `-alpha`
- B. the target branch name starts with `release`
- C. the target branch name starts with `release` but does not end with `-alpha`
...
- B. Use the `Disable workflow` option in GitHub Actions
...
- B. You define an output on workflow level in the `build` workflow
- C. All outputs are automatically passed to the caller workflows
- D. In the `build` workflow you write the output into `$GITHUB_OUTPUT` in one of the steps
...
- B. Use `concurrency` on workflow level
  ...
  ```
- C. Use `parallel` on workflow level
  ```yaml
  parallel: ${{ github.workflow }}
  ```
- D. Use `queue` on workflow level
  ```yaml
...
- B. Use concurrency
  ...
  concurrency:
    group: ${{ github.ref }}
  ```
- C. Use cancel-in-progress flag for `pull_request` event
  ```yaml
  on:
    pull_request:
      branches:
  ...
  ```
- D. Use concurrency with cancel-in-progress
  ```yaml
  concurrency:
...
- A. You cannot use `if: ${{ always() }}` and `needs` together. The workflow will fail on startup.
- B. job3 will run after job1 and job2 have been successfully completed
- C. job3 will run after both job1 and job2 have failed
- D. job3 will run after job1 and job2 have completed, regardless of whether they were successful
...
- B. All secrets available to `workflow A` will be also available to `workflow B` and `workflow C`
...
- A. When you want to save files produced by a job to view after a workflow run has ended, such as built binaries or build logs.
...
- D. When you want to reuse files that do change often between jobs or workflow runs, such as build dependencies from a package management system.
...
- B. Use artifacts to reuse files that don't change often between jobs or workflow runs, such as build dependencies from a package management system.
...
- A. Use the `actions/download-artifact` action with elevated permissions.
- B. Use the `actions/download-artifact` action and make sure the artifact is not expired
...
- C. cache the binaries in `deploy` and read the files from cache in `build`
- D. upload the binaries as artifacts in `deploy` and download them in `build`
...
- A. Starter workflows are a paid GitHub feature
- B. Your organization can create custom starter workflows for users in your organization
- C. They allow users to leverage ready-to-use (or requiring minimal changes) workflow templates
- D. Starter workflows are provided ready-to-use and cannot be modified or enhanced
- E. GitHub provides and maintains starter workflows for different categories, languages and tooling
...
- B. An environment in a repository
- C. A single repository
- D. The entire organization, or selected repositories in an organization
...
- D. `Python Actions`, `JavaScript Actions`, `Custom Actions`
...
- C. Only for Docker container actions
- D. False
...
- C. In the `action.yml` or `action.yaml` file in the action repository
- D. It's edited in GitHub Marketplace UI when published for sharing
...
- B. It will trigger two workflows, one with code from `commit A` and one with code from `commit B`
- C. It will run with code from `commit B`
...
- A. Using branch protection rules
...
- B. Each job in a workflow can reference a maximum of two environments.
- C. Each workflow can reference a single environment.
...
- A. Storing access keys in `secrets`
- B. Storing access keys in `variables`
- C. Using OIDC
- D. Using Vault
...
- C. Setup deployment protection rules for the repository
- D. Setup required approvals for fork runs in the repository
...
- A. `${{ secrets.SomeSecret }}` expression is only used for repository scoped secrets
- B. The secret `SomeSecret` is also declared in repository scope
- C. You need to use the GitHub API to access organization scoped secrets
- D. The secret `SomeSecret` is also declared in enterprise scope
...
- A. `echo "::debug::message=Watch out here!"`
...
- B. In the `_diag` folder directly on the runner machine
- C. In the job run logs of a job that ran on that Runner with debug logging enabled
...
- D. Using a GitHub provided script on the runner machine
...
- B. By creating the following conditional on job level
  ...
  my-job:
    ...
    if: ${{ secrets.MY_SECRET == '' }}
  ```
- C. Set the secret `MY_SECRET` as a job level environment variable, then reference that environment variable to conditionally run that step
  ```yaml
  my-job:
    runs-on: ubuntu-latest
    env:
...
- B. `HEAD /repos/{owner}/{repo}/actions/secrets/{secret_name}`
...
- F. By creating a repository secret with the same name `API_KEY`
- G. By creating a repository secret with the name `REPOSITORY_API_KEY`
- H. By creating a environment secret with the name `OVERRIDE_API_KEY`
...
- A. A self-hosted runner is a system to be able to create workloads automatically
- B. A self-hosted runner is a system to manage pull requests from users of the organization
- C. A self-hosted runner is a system that you deploy and manage to execute jobs from GitHub Actions on GitHub.com
- D. A self-hosted runner is a system to upload code to a private server
...
- D. Each workflow is composed of one or more jobs which is composed of one or more steps, and each step is an action or a script
...
- A. Scheduled workflows run on the latest commit on the main branch.
- B. Scheduled workflows run on the specific commit on the main branch.
- C. Scheduled workflows run on the specific commit on last modified branch.
- D. Scheduled workflows run on the latest commit on the repository default branch.
...
- A. set `working-directory` under `job`
  ```yaml
  defaults:
    run:
      shell: bash
...
- C. set `directory` under `job`
  ...
  job:
    directory: ./scripts
  ```
- D. set `directory` under `defaults.run`
  ```yaml
  defaults:
    run:
      shell: bash
...
- A. To trigger another workflow
- B. To set up dependencies
- C. To specify input parameters for an action
...
- C. Separating commands with a semicolon ;
- D. Using a multiline string with |
...
- B. Triggers workflows based on a schedule
- C. Allows defining multiple job configurations to run in parallel
- D. Sets environment variables for the job
...
- A. by automatically creating a new cache if the job is completed successfully
- B. by requiring manual intervention to create a new cache
- C. by terminating the workflow if a cache miss occurs
- D. by searching for a cache in other repositories
...
- B. add a condition in the workflow YAML for weekdays
...
- C. secrets larger than 48 KB cannot be stored
...
- A. `state()`, `always()`, `cancelled()` and `failure()`
- B. `success()`, `always()`, `cancelled()` and `failure()`
...
- A. depend on the behavior of the operating system in use
- B. treat environment variables as case-sensitive
...
- A. workflow jobs will fail if protection rules are configured
...
- D. workflow jobs won't start until all the environment's protection rules pass
...
- B. provide alternative keys to use in case of a cache miss
...
- A. it sets the timeout for waiting on external events before proceeding to the next step
- B. it limits the execution time for individual step
...
- C. inside a directory named `workflow-templates` within the current repository
...
- D. the `production-deploy` job will be marked as skipped
...
- D. by using the `matrix.property` syntax
...
- B. `runs.using` has `Dockerfile` as value
- C. `runs.using` has `docker` as value
...
- A. Default environment variables always have the prefix “GITHUB_”
...
- B. The entire workflow, by using `custom.env` at the top level of the workflow file
- C. All the jobs within a workflow, by using `jobs.env`
...
- A. Leave as is since access tokens will be passed automatically
  ```yaml
  with:
      repository: my-org/my-private-repo
      path: ./.github/actions/my-org/my-private-repo
  ```
- B. Create a GitHub secret `MY_ACCESS_TOKEN`
  ```yaml
  with:
      repository: my-org/my-private-repo
      path: ./.github/actions/my-org/my-private-repo
...
- B. `export API_VERSION=2.1 >> "$GITHUB_ENV"`
...
- B. No activity types are defined under the `pull_request` event.
- C. `on: schedule` was configured with `pull_requests: [reopened]`
- D. Branch protection rules were improperly configured.
...
- D. Only reusable workflows can accept inputs.
- E. Reusable workflows must be called on workflow job level (not from step-level).
- F. Composite actions are called via referencing the folder that contains their `action.yml` file.
...
- B. GitHub Enterprise Server instances are self-hosted, compared to GitHub Enterprise Cloud (GHEC) which is hosted and managed by GitHub.
...
- E. `actions/actions-sync` is primarily devoted to moving GitHub.com actions to a GHES instance.
...
- A. Commit SHAs are more convenient to use as opposed to tags
- B. Commit SHAs are guaranteed to point to the exact same code every time, tags are not
- C. Commit SHAs are more secure
- D. Commit SHAs are more difficult to trace in an audit, making it difficult for bad actors to determine how an action's code factors in overall processes.
- E. Commit SHAs are immutable, whereas tags have the potential to be changed
...
- C. Write the contents of a script block to the `GITHUB_SCRIPT` environmental variable
...
- C. Forked repositories do not inherit secrets from the original repository  
...
- D. There is no way to enforce this behavior, it cannot be worked around.
...
- A. Re-running a workflow with `Enable debug logging enabled`
- B. Setting a secret or variable named `ACTIONS_RUNNER_DEBUG` to `true`
...
- A. When using `hashFiles` as part of a cache key, if there is a cache miss, `hashFiles` gives additional debug info.  
- B. `hashFiles` is required for compatibility with Windows runners.
- C. If a cache key contains the dependencies file wrapped in `hashFiles`, the key changes when the dependencies file is updated, which helps keep it up to date.
...
- E. The `actions/create-github-app-token` can be called within workflows to create an installation access token, but the installation access token can only be used in future runs of the workflow.
...
- B. This cannot be done at an organizational level. All workflows that utilize `actions/upload-artifact` must use the required `retention-days` input.
...
- A. By utilizing the `retention-days` input in `actions/download-artifact`
...
- C. By navigating to the organization's Actions settings and editing the value of the "Artifact and log retention" setting
- D. By utilizing the `retention-days` input in `actions/upload-artifact` 
...
- B. By using the `actions/download-artifact` action in a workflow 
...
- A. In pull requests that have not been merged, `github.ref` is the fully-formed ref of the pull request title. 
- B. In pull requests that have been merged, `github.ref` refers to the fully-formed ref of the branch that was merged into.
- C. In pull requests (regardless of merge status), `github.ref` is the SHA of the last merge commit on the `GITHUB_REF` branch.
...
- E. In pull requests (regardless of merge status), `github.ref` refers to the pull request number 
...
- D. In push events, `github.ref` is the fully-formed ref of the branch or tag ref that was pushed. 
...
- B. `GITHUB_ENV`, because using it to set environmental variables puts significantly less strain on the runner, reducing workflow runtime.
- C. `GITHUB_ENV`, because job outputs can only be set and referenced within the same job.
- D. Job outputs, because the value of environmental variables set via writing to `GITHUB_ENV` only applies to the current job.
...
- B. starts with the same tools GitHub-hosted runners do, as well as a selected assortment of custom tools to enhance self-hosted runner management
...
- B. Docker container actions allow you to utilize Docker without requiring an `action.yml` file
- C. Docker container actions have fast startup speed on Windows and macOS runners
- D. Docker container actions are an out-of-the-box, low-overhead action
...
- A. `schedule` cannot be the only event in the workflow. It must be paired with a repository-based event, such as `push`
- B. The private repository containing the workflow has not had any repository activity in greater than 60 days, automatically disabling the workflow.
- C. The `cron` syntax is not scheduled correctly
- D. The workflow file must exist on the default branch in order to be triggered by the `schedule` event
- E. The `@daily` syntax was not used
...
- A. By remotely accessing self-hosted runners via SSH, navigating to the `.github/artifacts` directory, and deleting the selected artifacts
- B. By using the Github Actions UI to navigate to a workflow run and delete the artifacts individually
- C. By using the Github Actions UI to delete the workflow run that generated the artifacts
...
- A. Specify the the `pull_request` activity type as `closed` (no need for a job-level conditional)
  ```yaml
  on:
      pull_request:
          types: [closed]
...
- B.  Specify the `pull_request` activity type as `merged` (no need for a job-level conditional)
  ...
  jobs:
      post-merge:
  ``` 
- C.  Specify the the `pull_request` activity type as `closed` and use a job-level conditional to check if `github.ref` is equal to the merge branch of the pull request.
  ```yaml
  on:
      pull_request:
          types: [closed]
...
- D.  Specify the `pull_request` activity type as `merged`, and use a job-level conditional to check if `github.event.pull_request.merged` is true
  ...
  ``` 
- E.  Specify the `pull_request` activity type as `closed`, and use a job-level conditional to check if `github.event.pull_request.merged` is true
  ```yaml
  on:
      pull_request:
          types: [closed]
...
- E. OIDC involves the generation and use of short-lived tokens, which is more secure
...
- B. `CONTRIBUTING.md`
- C. `README.md`
- D. A `Dockerfile`, if the image is to be referenced from an image registry
- E. `.dockerignore`
- F. A `Dockerfile`, if the image is built as part of the action during the workflow run
...
- A. Communication logic with the corresponding GitHub Packages registry `https://npm.pkg.github.com`
...
- F. The workflow is triggered by a POST request to the workflow using the following endpoint `/repos/OWNER/REPO/actions/workflows/<WORKFLOW_ID>/dispatches` 
...
- A. JavaScript actions should be used for short inline scripts
- B. JavaScript actions should be used when you want a custom reusable action to be used across repositories 
- C. `actions/github-script` should be used for short inline scripts
...
- E. JavaScript actions should be used when you want a low-overhead solution to making GitHub API calls.
...
- A. Switch to GitHub-hosted standard runners, since self-hosted runners will be blocked if IP allow lists are enabled
...
- C. The job will run on a self-hosted runner that has all the labels applied.
...
- A. Re-running a workflow ensures `GITHUB_TRIGGERING_ACTOR` remains unchanged, so it is unambiguous as to who originally triggered the workflow
- B. Re-running a workflow allows you to enable extra debug logging for the selected job(s).
- C. Re-running a workflow means the workflow jobs run in the same context of the commit SHA and git ref of the original event that triggered the job
- D. Re-running a workflow ensures `GITHUB_ACTOR` is updated, so it is unambiguous as to who re-ran the workflow
- E. Re-running a workflow lets you re-run failed workflow jobs, as opposed to generating a new run which will run all jobs.
...
- A. One self-hosted runner per repository, set up at the repository level
- B. GitHub-hosted runners, with all workflows utilizing `actions/setup-node`
...
- E. GitHub-hosted runners set up at the organization-level
...
- C. Both the caller and reusable workflow will not get called, because `issues` is not an available trigger for GitHub Actions. 
- D. The reusable workflow will not be called, since reusable workflows must be in a subfolder of `.github/workflows`
...
- A. The Python script will run successfully, because the `chmod` command grants execute permissions to the script.
- B. The Python script will not run, because `runs-on` does not have a value of `python`.
...
- A. `push` and `weekly`
- B. `push` and `schedule`
- C. This is not possible: `schedule` cannot be combined with other events
- D. `push` and `workflow_dispatch`
- E. `pull_request` (with `types:[closed]`) and `schedule`
...
- A. Only individual users can be assigned as required reviewers, not teams.
...
- E. Required reviewers need at least `write` access to the repository in order to approve.
...
- E. The README is thorough in defining the purpose of the action and how it works
