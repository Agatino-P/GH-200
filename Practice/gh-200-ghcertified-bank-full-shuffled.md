...
> **🔀 Shuffled 2026-07-02** — option order randomized per question (`shuffle_all.py`, seed 20260702) and answer keys remapped accordingly. Prose notes may still reference pre-shuffle option letters.

## Q001

Which statement is correct regarding passing permissions to reusable workflows?

- A. The `GITHUB_TOKEN` permissions passed from the caller workflow can be neither downgraded or elevated by the called workflow.
...
- B. Yes, when using standard GitHub-hosted runners
- C. No, all GitHub Actions usage is billed
- D. Yes, but only for the first 2,000 minutes per month
...
- B. Adding a label to a pull request
...
- D. Workflows can be triggered manually, by an event or run on a schedule
- E. Workflows are written in any of `.yaml`, `.json` or `.toml` formats
  > Workflows can only be defined in `.yaml` format
- F. Workflow can run only one job at a time
- G. Workflows can be shared in GitHub Marketplace
  > Actions (not workflows) can be shared in GitHub Marketplace

**Answer:** A, C, D  ·  *multi-select (3 correct)*
...
- A. Workflow name
- B. One or more jobs
- C. Defined branches on which the workflow will run
...
- C. Only the first job runs, others require manual approval
- D. Jobs run based on the order they are defined in the workflow file
...
- A. use the `requires` keyword in job A to create this dependency
- B. use the `requires` keyword in job B to create this dependency
...
- A. Using the `matrix` context
- B. Using the `vars` context
...
- B. Using glob patterns
- C. Using `branches` filter
- D. You create the workflow only on `prod` branch
...
- B. the source branch name starts with `release`
- C. the target branch name starts with `release`
- D. the source branch name starts with `release` but does not end with `-alpha`
...
**Answer:** B, C, D, E, F  ·  *multi-select (5 correct)*
...
- A. Delete environment that is required for this workflow
- B. Prevent any new commits to main branch
- C. Use the `Disable workflow` option in GitHub Actions
- D. Remove secrets that are required for this workflow
...
- A. Reacting to new activity on a repository (e.g new contributor)
- B. Checking if the activity comes from an user or a bot
- C. Limiting workflow runs to specific activity types using the `types` filter
...
- A. You define an output on job level in the `build` workflow
- B. In the `build` workflow you write the output into `$GITHUB_OUTPUT` in one of the steps
- C. You define an output on workflow level in the `build` workflow
...
- C. Using defaults.env on workflow level to set default environment variables for an entire workflow
  > There is no such thing as defaults.env
- D. Using defaults.run on job level to set default working-directory for all steps in a single job
- E. Using defaults.run on workflow level to set default shell (e.g bash) for an entire workflow

**Answer:** D, E  ·  *multi-select (2 correct)*
...
- A. Use `queue` on workflow level
  ...
  ```
- B. Use `concurrency` on workflow level
  ```yaml
...
- A. job3 will run after both job1 and job2 have failed
- B. job3 will run after job1 and job2 have completed, regardless of whether they were successful
- C. You cannot use `if: ${{ always() }}` and `needs` together. The workflow will fail on startup.
- D. job3 will run after job1 and job2 have been successfully completed
...
- A. `if: ${{ github.org == 'my-org' && github.repository == 'my-repo' }}`
  > https://docs.github.com/en/actions/learn-github-actions/contexts#github-context
- B. `if: ${{ github.organization == 'my-org' && github.repository == 'my-repo' }}`
  > https://docs.github.com/en/actions/learn-github-actions/contexts#github-context
- C. `if: ${{ github.repository == 'my-org/my-repo' }}`
- D. `if: github.repository == 'my-org/my-repo'`

**Answer:** C, D  ·  *multi-select (2 correct)*
...
- A. Reference a version tag
- B. Reference the main branch
- C. Reference the commit SHA
...
- A. `continue-on-error` flag in the failing step
  ...
  ```
- B. `failure()` conditional in the failing step
  ```yaml
  steps:
      - uses: my-org/failing-action@v1
        if: failure()
  ```
- C. `ignore-error` flag in the failing step
  ```yaml
  steps:
      - uses: my-org/failing-action@v1
        ignore-error: true
...
- A. `gh set-output "PET=DOG"`
- B. `echo "DOG=PET" >> "$GITHUB_OUTPUT"`
- C. `gh set-output "DOG=PET"`
- D. `echo "PET=DOG" >> "$GITHUB_OUTPUT"`
...
- A. All secrets from `octo-org` organization and `octo-org/example-repo` repository will be available to `workflow B`, but not to `workflow C`
  > Not all secrets from `octo-org` organization have to be made available to `octo-org/example-repo`.
- B. Only repository and environment secrets available to `workflow A` will be available to `workflow B`, but not to `workflow C`. Organization scoped secrets cannot be inherited
...
- A. Use artifacts to reuse files that don't change often between jobs or workflow runs, such as build dependencies from a package management system.
...
- C. Use artifacts to save files produced by a job to view after a workflow run has ended, such as test results or build logs.
...
- C. Yes, all branches can restore caches created on the default branch
...
- B. You cannot access `artifacts` that were created in a different workflow run
- C. Use the `actions/download-artifact` action with elevated permissions.
- D. Use the `actions/upload-artifact` action.
...
- A. cache the binaries in `build` and read the files from cache in `deploy`
- B. upload the binaries as artifacts in `build` and download them in `deploy`
- C. upload the binaries as artifacts in `deploy` and download them in `build`
...
- A. GitHub provides and maintains starter workflows for different categories, languages and tooling
- B. Starter workflows are provided ready-to-use and cannot be modified or enhanced
  > https://docs.github.com/en/actions/using-workflows/using-starter-workflows#using-starter-workflows
- C. They allow users to leverage ready-to-use (or requiring minimal changes) workflow templates
- D. Your organization can create custom starter workflows for users in your organization
...
- B. The entire organization, or selected repositories in an organization
- C. An environment shared across multiple repositories
...
- A. `Docker container actions`, `Java Actions`, `Composite Actions`
...
- A. In the `action.yml` or `action.yaml` file in the action repository, but it is not required if the action is not meant to be shared and used by the public
  > All actions require the metadata file.
- B. It's edited in GitHub Marketplace UI when published for sharing
...
- B. You cannot re-run workflows in GitHub Actions. You have to trigger a new workflow which will run with latest changes
- C. It will trigger two workflows, one with code from `commit A` and one with code from `commit B`
- D. It will run with code from `commit A`
...
- C. Each workflow can reference a maximum of two environments.
- D. Each job in a workflow can reference a maximum of two environments.
...
- B. `GITHUB_USER`
- C. `GITHUB_WORKFLOW`
- D. `GITHUB_ORGANIZATION`
- E. `GITHUB_ACTOR`
- F. `GITHUB_TOKEN`

**Answer:** A, C, E  ·  *multi-select (3 correct)*
...
- B. `echo "::debug::message=Watch out here!"`
- C. `echo ":debug:Watch out here!"`
...
- C. GitHub Enterprise Server has access to all GitHub.com Actions by default
...
- A. In the job run logs of a job that ran on that Runner with debug logging enabled
...
- A. By trying to access the runner machine by `ssh` to validate the network connectivity
...
- B. It's not possible because configuration variables cannot be used in `if` conditionals
  > That is true for `secrets` but not for configuration variables
...
- A. By creating the following conditional on step level
  ```yaml
  my-job:
    runs-on: ubuntu-latest
    steps:
...
- B. Set the secret `MY_SECRET` as a job level environment variable, then reference that environment variable to conditionally run that step
  ```yaml
  my-job:
    runs-on: ubuntu-latest
    env:
...
- B. By creating a environment secret with the name `ENVIRONMENT_API_KEY`
- C. By creating a repository secret with the name `REPOSITORY_API_KEY`
- D. By creating a repository secret with the same name `API_KEY`
- E. By creating a enterprise secret with the same name `API_KEY`
- F. By creating a environment secret with the name `OVERRIDE_API_KEY`
- G. By creating a enterprise secret with the name `OVERRIDE_API_KEY`
- H. By creating a repository secret with the name `OVERRIDE_API_KEY`

**Answer:** A, D  ·  *multi-select (2 correct)*
...
- G. Self Hosted Runners

**Answer:** A, B, C, G  ·  *multi-select (4 correct)*
**Docs:** — (no link in source)
...
- A. No
- B. Only if they run in parallel
...
- B. Each action is composed of one or more workflows which is composed of one or more jobs, and each job is composed of one or more steps
...
- D. Each workflow is composed of one or more actions which is composed of one or more jobs, and each job is composed of one or more steps
...
- A. set `directory` under `job`
  ...
  job:
    directory: ./scripts
  ```
- B. set `working-directory` under `defaults.run`
  ```yaml
  defaults:
    run:
      shell: bash
...
- A. Triggers a job based on an event
...
- A. Using a multiline string with |
- B. Defining commands in an array
- C. Using && to chain commands
...
- C. Using the actions/cache action
...
- A. Sets environment variables for the job
...
- A. You can add a self-hosted runner to an organization
- B. You can add a self-hosted runner to a repository
- C. You can add a self-hosted runner to an enterprise
- D. You can add a self-hosted runner to a step
...
- A. `status()`, `always()`, `cancelled()` and `failure()`
- B. `state()`, `always()`, `cancelled()` and `failure()`
- C. `success()`, `always()`, `cancelled()` and `failure()`
- D. `completed()`, `always()`, `cancelled()` and `failure()`
...
- A. the workflow will run when either `branches` or `paths` are satisfied, but will only apply the matching filter
- B. the workflow will not run when both `branches` and `paths` are satisfied
- C. the workflow will only run when both `branches` and `paths` are satisfied
...
- B. workflow jobs will fail if protection rules are configured
- C. workflow jobs will start immediately and protection rules are evaluated during execution
- D. workflow jobs will start if at least one protection rule passes
...
- A. specify the location of the cached files
- B. enable cross-OS cache functionality
- C. indicate whether a cache hit occurred
...
- C. `ACTIONS_WORKFLOW_DEBUG`
- D. `ACTIONS_STEP_DEBUG`
...
- A. it specifies the maximum duration a job is allowed to run
- B. it sets the timeout for waiting on external events before proceeding to the next step
- C. it limits the execution time for individual step
- D. it defines the time interval for individual commands within a step
...
- D. inside a directory named `.github/org-templates`
...
- B. the `production-deploy` job will run if the `node-version` is `14`
- C. the `production-deploy` job will be marked as skipped
...
- A. by using the `context` keyword within the job configuration
- B. by accessing the variables directly with the syntax `version` and `os`
...
- B. After the workflow run has completed, regardless of its age.
...
- C. Workflow runs cannot be deleted, but they can be archived.
...
- A. By including any one of the following keywords in the commit message or in the title of the pull-request
  ...
  [actions skip]
  ```
- B. The above workflow will run in every event of push or pull request in every case
- C. Provide `SKIP_WORKFLOW` in the commit message
...
- B. Default environment variables always have the prefix “GITHUB_”
- C. You can add a new default environment variable adding the prefix “GITHUB_” to it
- D. Most of the default environment variables have a corresponding context property
- E. Default environment variables can be accessed using the env context
- F. Default environment variables are set by GitHub and not defined in a workflow

**Answer:** A, D, F  ·  *multi-select (3 correct)*
...
- D. The entire workflow, by using `custom.env` at the top level of the workflow file
- E. A specific environment in the repository, by using `environment.<environment_id>.env` at the top level of the workflow file
- F. The contents of a job within a workflow, by using `jobs.<job_id>.env`

**Answer:** A, C, F  ·  *multi-select (3 correct)*
...
- B. Leave as is since access tokens will be passed automatically
  ...
  with:
      ...
      path: ./.github/actions/my-org/my-private-repo
  ```
- C. Create an input `MY_ACCESS_TOKEN`
  ```yaml
  with:
      repository: my-org/my-private-repo
      path: ./.github/actions/my-org/my-private-repo
...
- A. 5 jobs
- B. 4 jobs
- C. No jobs will run because the syntax is invalid.
- D. 7 jobs
- E. 6 jobs
...
**Answer:** B, C, D  ·  *multi-select (3 correct)*
...
- C. `echo "API_VERSION=2.1" >> "$GITHUB_ENV"`
...
- A. Only with elevated permissions
- B. True
- C. False
...
- B. A workflow-level `outputs` block must have the following structure:
  ```
  outputs:
      <output-name>
...
- E. A workflow-level `outputs` block should only be used in reusable workflows, not caller workflows.
...
- A. Reusable workflows are called via referencing the folder that contains their `action.yml` file.
  > Reusable workflows are regular `.yml` or `.yaml` files that are stored in `.github/workflows`. They do not have an `action.yml` file.
- B. Secrets can be passed to both reusable workflows and calling composite actions via the `uses.secrets` block.
...
- D. Composite actions must be called as a step within a job
...
- E. Only reusable workflows can accept inputs.
  > Both reusable workflows and composite inputs can accept inputs. 
- F. Reusable workflows must be called on workflow job level (not from step-level).
...
**Answer:** C, D, F, G  ·  *multi-select (4 correct)*
...
- C. Using GitHub Connect, users can follow a manual process to access GitHub.com actions. This process must be done once per desired action.
...
**Answer:** A, D, E  ·  *multi-select (3 correct)*
...
- A. Commit SHAs are guaranteed to point to the exact same code every time, tags are not
- B. Commit SHAs are more difficult to trace in an audit, making it difficult for bad actors to determine how an action's code factors in overall processes.
...
- A. Write the contents of a script block to the `GITHUB_SCRIPT` environmental variable
...
- C. Via the `actions/github-script` action
...
- B. There is no way to enforce this behavior, it cannot be worked around.
- C. Nothing needs to be done, since `jobs.<job_id>.strategy.matrix.fail-fast` has a default setting of `false`
...
- E. Nothing needs to be done, since `jobs.<job_id>.strategy.fail-fast` has a default setting of `false`
...
- A. By adding a `runner-diagnostic-logs` subfolder to the `_diag` directory of the self-hosted runner being used
...
- D. Setting a secret or variable named `ACTIONS_RUNNER_DEBUG` to `true`
...
- B. Use shell conditionals in combination with `jobs.<job_id>.steps[*].if`
  ```yaml
...
- C. Use the built-in `startsWith` method in combination with `jobs.<job_id>.steps[*].if`
  ...
  ```
- D. Use the `branches` filter under `workflow_call`
  ```yaml
  on:
    workflow_call:
      branches:
...
- D. `hashFiles` is required for compatibility with Windows runners.
...
- A. `GITHUB_TOKEN` is a type of installation access token.
...
- B. Installation access tokens cannot be configured to act on behalf of their associated Github App. 
...
- C. The `actions/create-github-app-token` can be called within workflows to create an installation access token, but the installation access token can only be used in future runs of the workflow.
...
- D. The `actions/create-github-app-token` can be called within workflows to create an installation access token available for immediate use. 
...
- C. This cannot be done: artifacts are strictly stored for 90 days across all systems implementing Github Actions. 
...
- D. By using self-hosted runners, creating a `.github/retention-policy.yml` file, and specifying the value of the `artifact-retention-period` key 
...
- B. By using a specific GitHub API endpoint
...
- C. By remotely accessing self-hosted runners via SSH and accessing the `.github/artifacts` directory
...
- D. By using the `actions/download-artifact` action in a workflow 
- E. By downloading artifacts from the Github Actions UI workflow run
...
**Answer:** B, D, E  ·  *multi-select (3 correct)*
**Docs:** https://github.com/actions/upload-artifact#inputs
...
- C. In pull requests that have been merged, `github.ref` refers to the fully-formed ref of the branch that was merged into.
...
- E. In push events, `github.ref` is the message of the commit that triggered the workflow.
...
- A. Prints this line as a step-level debug message
...
- D. Adds this line to the job summary
...
- B. Navigating to the "Webhooks" section of the repository settings 
...
- C. Checking the "Show event webhook payload" checkbox under the workflow run options.
- D. Setting a secret or variable named `SHOW_EVENT_PAYLOAD` to `true`
...
- A. starts with the same tools GitHub-hosted runners do, as well as a selected assortment of custom tools to enhance self-hosted runner management
- B. starts off empty and must be populated in order to save tools between runs
...
- C. cannot be populated
- D. starts off the same as GitHub-hosted runners in that it is pre-populated with certain tools
...
**Answer:** B, F  ·  *multi-select (2 correct)*
**Docs:** https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#pull_request
...
- A. Docker container actions are an out-of-the-box, low-overhead action
...
- A. The `@daily` syntax was not used
...
- C. The private repository containing the workflow has not had any repository activity in greater than 60 days, automatically disabling the workflow.
...
- A. By using the Github Actions UI to delete the workflow run that generated the artifacts
  > Deleting a workflow run also deletes the artifacts associated with the run. 
- B. By using the Github Actions UI to navigate to a workflow run and delete the artifacts individually
...
- E. By using the `actions/delete-artifact` action in a workflow 
- F. By remotely accessing self-hosted runners via SSH, navigating to the `.github/artifacts` directory, and deleting the selected artifacts
...
- B. Both `pull_request` and `pull_request_target` events have default activity types of `opened`, `synchronize`, and `reopened`.
- C. Workflows will not run on `pull_request` activity if there is a merge conflict
...
- E. `pull_request` should be used with caution, since PRs from forks will allow the workflow to access all secrets within the repository due to being associated with the default branch.
...
**Answer:** B, C, F  ·  *multi-select (3 correct)*
**Docs:** https://docs.github.com/en/actions/reference/workflows-and-actions/events-that-trigger-workflows#pull_request
...
- B. Cloud providers require the use of OIDC.
  > OIDC is optional and recommended, but not strictly required.
- C. OIDC prevents you from having to keep cloud credentials as long-lived GitHub secrets 
- D. OIDC generates JSON web tokens (JWTs) that can be used across workflow jobs
  > OIDC infrastructure involves creating JWTs that are unique for each workflow job
- E. Using OIDC allows you to circumvent setting up trust policies with cloud providers
...
- A. The workflow is triggered by a POST request to the workflow using the following endpoint `/repos/OWNER/REPO/actions/workflows/<WORKFLOW_ID>/dispatches` 
...
- B. The `on.repository_dispatch.event_types` workflow key corresponds to the `event_type` parameter in the request payload, restricting the workflow to only trigger on relevant external events
...
- D. The external system makes a POST request to the GitHub API to create a repository dispatch event.
- E. The workflow is triggered by the creation of a repository dispatch event 
...
- F. The external system makes a PUT request to the GitHub API to create a repository dispatch event
  > The proper HTTP method for the "Create a repository dispatch event" is POST.

**Answer:** C, D, E  ·  *multi-select (3 correct)*
...
- B. `actions/github-script` should be used for short inline scripts
- C. JavaScript actions should be used for short inline scripts
- D. JavaScript actions should be used when you want a low-overhead solution to making GitHub API calls.
...
- E. `actions/github-script` should be used when you want to use a pre-authenticated client to interact with the GitHub API.
- F. `actions/github-script` should be used when you need to utilize a fine-tuned Node.js environment with several specific dependencies
...
- C. No, because while workflow runs will remain after a workflow is deleted, generated artifacts become corrupted
- D. Yes, but only if she has administrator privileges
...
- C. Adding the self-hosted runners' IP address(es) to the organization's IP allow list
...
- E. Adding the `.ip-exception` file to the top-level of the self-hosted runner's directory structure
...
- A. The job will run on a self-hosted runner that has any of the labels applied.
...
- B. Re-running a workflow ensures `GITHUB_TRIGGERING_ACTOR` remains unchanged, so it is unambiguous as to who originally triggered the workflow
...
- D. Re-running a workflow means the workflow jobs run in the same context of the commit SHA and git ref of the original event that triggered the job
- E. Re-running a workflow overwrites the failing job runs, making runs appear more straightforward.
...
- F. Re-running a workflow allows you to enable extra debug logging for the selected job(s).

**Answer:** C, D, F  ·  *multi-select (3 correct)*
...
- C. Create a new runner label, add the labels to the runners, then select which repositories are allowed access to the label in the label settings.
...
- D. One self-hosted runner per repository, set up at the repository level
...
- E. Self-hosted runners set up at the organization-level
...
- C. The Python script will not run, because `actions/checkout` is not included in the workflow.
...
- A. `pull_request` (with `types:[closed]`) and `schedule`
...
- C. You can prevent self-reviews in the event the person who wants to deploy is also a required reviewer.
...
- B. The `action.yml` is very brief
- C. A 'Verified Creator' badge on the Marketplace page for the action
